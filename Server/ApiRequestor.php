<?php
/**
 * Send request to RestCall API
 * Better don't use this class directly, use RestCall_VtWeb, RestCall_VtDirect, RestCall_Transaction
 */

class RestCall_ApiRequestor {
	...
	public static function remoteCall($url, $server_key, $data_hash, $post = true)
	{
		...
		// merging with RestCall_Config::$curlOptions
		if (count(RestCall_Config::$curlOptions)) {
			// We need to combine headers manually, because it's array and it will no be merged
			if (RestCall_Config::$curlOptions[CURLOPT_HTTPHEADER]) {
				$mergedHeders = array_merge($curl_options[CURLOPT_HTTPHEADER], RestCall_Config::$curlOptions[CURLOPT_HTTPHEADER]);
				$headerOptions = array( CURLOPT_HTTPHEADER => $mergedHeders );
			} else {
				$mergedHeders = array();
			}

			$curl_options = array_replace_recursive($curl_options, RestCall_Config::$curlOptions, $headerOptions);
		}
		...
		if ($result === FALSE) {
			...
			if (!in_array($result_array->status_code, array(200, 201, 202, 407))) {
				$message = 'RestCall Error (' . $result_array->status_code . '): '
